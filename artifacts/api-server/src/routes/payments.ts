...
import { paymentsTable, ordersTable, orderItemsTable, restaurantTablesTable, staffTable, menuItemsTable, notificationsTable } from "@workspace/db";
...
router.post(
  ...
  const processor = await resolveStaffActor(req.user!, processedBy);
  if (!processor) {
