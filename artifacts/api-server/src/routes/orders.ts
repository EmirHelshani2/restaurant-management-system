...
import { resolveStaffActor } from "../lib/staff";
...
router.get(
  "/orders",
  requireAuth,
  requireRole("admin", "manager", "waiter", "cashier"),
  ...
  if (req.user?.role === "waiter") {
    ...
    if (!actor) {
      res.status(400).json({ error: "No staff record is linked to this waiter account" });
      return;
    }

    orders = orders.filter((order) => order.waiterId === actor.id);
  }
...
router.post(
  ...
  const waiter = await resolveStaffActor(req.user!, waiterId);
  if (!waiter) {
    res.status(400).json({ error: "No staff record is linked to this account" });
    return;
  }
  ...
  const [order] = await db.insert(ordersTable).values({
    tableId, waiterId: waiter.id, notes, status: "open",
    subtotal: "0", tax: "0", serviceCharge: "0", total: "0",
  }).returning();

  await db.update(restaurantTablesTable).set({
    status: "occupied", waiterId: waiter.id, activeOrderId: order.id, updatedAt: new Date(),
  }).where(eq(restaurantTablesTable.id, tableId));
...
router.get(
  ...
  if (req.user?.role === "waiter") {
    const actor = await resolveStaffActor(req.user);
    if (!actor || order.waiterId !== actor.id) {
...
router.patch(
  ...
  if (req.user?.role === "waiter") {
    const actor = await resolveStaffActor(req.user);
    if (!actor || existingOrder.waiterId !== actor.id) {
      res.status(403).json({ error: "Insufficient permissions" });
      return;
    }
  }
...
router.patch(
  ...
  requireAuth,
  requireRole("admin", "manager", "kitchen", "bar", "waiter"),
  async (req, res): Promise<void> => {
  const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id, 10);
  ...
  // Waiters can only mark items as served
  if (req.user?.role === "waiter" && status !== "served") {
    res.status(403).json({ error: "Waiters can only mark items as served" });
    return;
  }
  ...
  if (req.user?.role === "kitchen" && menuItem.department !== "kitchen") {
    res.status(403).json({ error: "Kitchen staff can only update kitchen items" });
    return;
  }

  if (req.user?.role === "bar" && menuItem.department !== "bar") {
    res.status(403).json({ error: "Bar staff can only update bar items" });
    return;
  }

  if (status === "ready") {
    const dept = menuItem.department;
    await db.insert(notificationsTable).values({
      type: `${dept}:item_ready`,
      message: `${menuItem.name} is ready for order #${item.orderId}`,
      targetRole: "waiter",
    });
  }
