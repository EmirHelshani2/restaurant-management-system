import { useListBarOrders, useUpdateOrderItemStatus, OrderItemStatus } from "@workspace/api-client-react";
...
export default function Bar() {
  const { data: tickets, isLoading } = useListBarOrders({
  ...
  const handleStatusChange = (orderId: number, itemId: number, status: OrderItemStatus) => {
    updateStatusMutation.mutate({ orderId, itemId, data: { status } });
  };

  if (isLoading) return <div className="p-8">Loading bar tickets...</div>;

  return (
    <div className="p-8 space-y-8 max-w-full overflow-x-hidden">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Bar Display</h1>
        <p className="text-muted-foreground">Auto-refreshing drinks queue</p>
      </div>
